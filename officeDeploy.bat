...
if not defined o16c2r goto :oh_o15c2r
call :oh_activate_o16c2r
:oh_o15c2r
:: Process Office 15.0 C2R
if not defined o15c2r goto :oh_msi
call :oh_activate_o15c2r
:oh_msi
:: Process MSI versions
if defined o14msi call :oh_setspp 14 & call :oh_processmsi 14 %o14msi_reg%
if defined o15msi call :oh_setspp & call :oh_processmsi 15 %o15msi_reg%
if defined o16msi call :oh_setspp & call :oh_processmsi 16 %o16msi_reg%
:: Cleanup
call :oh_clearblock
call :oh_uninstkey
call :oh_licrefresh
...
echo Office is permanently activated.
exit /b 0
) else (
...
echo Checking Installed Products             [Product IDs not found. Aborting activation...]
set error=1
exit /b
)
if defined noOsppc (
echo Checking OSPPC.DLL                      [Not found. Aborting activation...]
set error=1
exit /b
)
...
::  Add registry to prevent non-genuine banner
...
if not "%osarch%"=="x86" (reg delete "%kmskey%" /f /reg:32 %nul% & reg add "%kmskey%" /f /v KeyManagementServiceName /t REG_SZ /d "10.0.0.10" /reg:32 %nul%)
reg delete "%kmskey%" /f %nul% & reg add "%kmskey%" /f /v KeyManagementServiceName /t REG_SZ /d "10.0.0.10" %nul%
)
)
exit /b

::  Activate Office 15.0 C2R
:oh_activate_o15c2r
call :oh_reset
call :dk_actids 0ff1ce15-a989-479d-af46-f275c6370663
set oVer=15
...
if not !keyerror! EQU 0 (
call :dk_keyinstalled
if defined _keyalready set keyerror=0
)
)
